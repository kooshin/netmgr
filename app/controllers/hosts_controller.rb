class HostsController < ApplicationController
  ...
  def index
    q = params[:q] || {}
    q[:s] = 'hostname asc' unless q.has_key?(:s)

    @ransack = Host.ransack(q)
    @hosts = @ransack.result
  end
  ...
  private
    ...
    def host_params
      params.require(:host).permit(:hostname, :connect, :username, :password, :enable_password)
    end
end
